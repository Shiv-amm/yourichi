...
function charData() {
  ...
  char.forEach(character => {
    character.addEventListener('click', () => {
      ...
      // Attach the event listener for the back arrow
      const closeBtn = dataContainer.querySelector('.header i');
      closeBtn.addEventListener('click', () => {
        containerData.classList.add('hide'); // Hide the character details
      });
    });
  });
}
